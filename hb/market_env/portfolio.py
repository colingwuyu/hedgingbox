...
class Portfolio():
    ...
    def rebalance(self, actions):
        ...
        cashflows = 0.
        trans_costs = 0.
        trunc_actions = self._risk_limits.review_actions(actions, self)
        for i, action in enumerate(trunc_actions):
            # rebalance hedging positions
            proceeds, trans_cost = self._hedging_portfolio[i].buy(action)
